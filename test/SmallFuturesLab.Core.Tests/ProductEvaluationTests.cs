...
public sealed class ProductEvaluationTests
{
    ...
    public void TickValue_Equals_TickSize_Times_Multiplier()
    {
        ...
        var eval = new ProductEvaluation(product, 10_000, CreateRiskConfig());
    ...
    {
        var product = CreateProduct(price: 2500, multiplier: 10, marginRate: 0.1);
        var riskConfig = CreateRiskConfig(lots: 2);
        var eval = new ProductEvaluation(product, 10_000, riskConfig);
    ...
    {
        var product = CreateProduct(tickSize: 1, multiplier: 10);
        var riskConfig = CreateRiskConfig(stopTicks: 10, lots: 2);
        var eval = new ProductEvaluation(product, 10_000, riskConfig);
    ...
    {
        var product = CreateProduct(tickSize: 1, multiplier: 10);
        var riskConfig = CreateRiskConfig(slippageTicks: 2, lots: 2);
        var eval = new ProductEvaluation(product, 10_000, riskConfig);
    ...
    public void CostMoney_Equals_RoundTripFee_Times_Lots_Plus_SlippageMoney()
    {
        ...
        var riskConfig = CreateRiskConfig(slippageTicks: 2, lots: 1);
        var eval = new ProductEvaluation(product, 10_000, riskConfig);
    ...
    {
        var product = CreateProduct(tickSize: 1, multiplier: 10, roundTripFee: 6);
        var riskConfig = CreateRiskConfig(stopTicks: 10, slippageTicks: 2, lots: 1);
        var eval = new ProductEvaluation(product, 10_000, riskConfig);
    ...
    {
        var product = CreateProduct(tickSize: 1, multiplier: 10, roundTripFee: 6);
        var riskConfig = CreateRiskConfig(stopTicks: 100, slippageTicks: 0, lots: 1);
        var eval = new ProductEvaluation(product, 10_000, riskConfig);
    ...
    public void Evaluate_Returns_Rejected_When_CostRatio_Exceeds_RejectCostRatio()
    {
        ...
        var riskConfig = CreateRiskConfig(stopTicks: 10, slippageTicks: 0, lots: 1);
        var eval = new ProductEvaluation(product, 100_000, riskConfig);
    ...
    {
        var product = CreateProduct(tickSize: 1, multiplier: 10, roundTripFee: 6);
        var riskConfig = CreateRiskConfig(stopTicks: 15, slippageTicks: 0, lots: 1);
        var eval = new ProductEvaluation(product, 10_000, riskConfig);
        var accountConfig = new AccountRiskConfig { AccountEquity = 10_000 };
    ...
    {
        var product = CreateProduct(tickSize: 1, multiplier: 10, roundTripFee: 6);
        var riskConfig = CreateRiskConfig(stopTicks: 5, slippageTicks: 0, lots: 1);
        var eval = new ProductEvaluation(product, 100_000, riskConfig);
        var accountConfig = new AccountRiskConfig { AccountEquity = 100_000 };
    ...
    private static ProductInfo CreateProduct(
        double price = 2500,
    ...
    private static ProductRiskConfig CreateRiskConfig(
        int stopTicks = 10,
        int slippageTicks = 2,
        int lots = 1)
    {
        return new ProductRiskConfig
        {
            StopTicks = stopTicks,
